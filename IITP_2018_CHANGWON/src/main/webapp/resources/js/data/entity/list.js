...
function bindToggleCheckbox() {
	...
	$("input[name=recordIds]:checkbox").click(function(){
		if($("input[name=recordIds]:checkbox").length == $('input[name=recordIds]:checkbox:checked').length){
...
function bindAccrodianList() {
	$('.btn_label_onoff').click(function(){
		...
		if(!$(this).closest('li').hasClass('on')) {
			...
			$(this).closest('.label_accordion').find('.btn_icon_upload_wihte').hide();
			...
			if($(this).closest('li').hasClass('on')) {
				$(this).closest('li').find('.cont_tit3').find('.btn_icon_download_white').show();
				$(this).closest('li').find('.cont_tit3').find('.btn_icon_upload_wihte').show();
			}
	...
	var groupInfo = $('#groupName').val();
	$('.label_accordion').children().each(function(){
		var id = $(this).attr('id');
		
		if(groupInfo == id){
			$(this).find('.cont_gray').slideUp('fast');
			$(this).closest('.label_accordion').removeClass('on');
			$(this).find('.btn_label_onoff').attr('title','펼치기');
			$(this).find('.btn_icon_download_white').hide();
			$(this).find('.btn_icon_upload_wihte').hide();
			$(this).addClass('on');
			$(this).find('.cont_gray').show();
			$(this).attr('title','');
			
			
			$(this).find('.cont_tit3').find('.btn_icon_download_white').show();
			$(this).find('.cont_tit3').find('.btn_icon_upload_wihte').show();
		}
	});
}
...
function fn_excelDown(groupName) {
...
function fn_upload(groupName) {
	$('#uploadFile').click();
	$('#uploadOpt').val(groupName);
}

function fn_editEntity(){
	var groupName = $('#groupName').val();
	var entityTree = $('#'+groupName+'_tree_list');
	var name = entityTree.jstree(true).get_selected();
	
	var url = contextPath+'/data/entity/entityEdit.do?';
	url += 'groupName='+groupName;
	url += '&name='+name[0];
	url += '&selIds='+name[0];
	
	window.open(url, 'edit_entity_tag', 'width=827,height=225')
}
