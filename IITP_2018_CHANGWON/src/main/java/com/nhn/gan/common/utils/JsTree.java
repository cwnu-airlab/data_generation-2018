...
//    @Slf4j
    ...
    public static class Mapper {
        ...
        public List<Node> parse(List<?> items) {
            ...
            Map<String, Set<String>> treeInfo = new HashMap<>();
            for(String name : names.keySet()) {
            	 String parentName = getParentName(name);
            	 parentName = parentName == null ? ROOT_PARENT : parentName;
            	 String subName = name.substring(parentName.length());
            	 Set<String> childInfo = null;
            	 if(!treeInfo.containsKey(parentName)) {
            		 childInfo  = new HashSet<>();
            	 } else {
            		 childInfo = treeInfo.get(parentName);
            	 }
            	 childInfo.add(subName);
            	 treeInfo.put(parentName, childInfo);
            }
            ...
            for (String name : names.keySet()) {
                ...
                Set<String> childInfo = treeInfo.get(name);
		...
		public String parseAsHtml(List<CollectionVo> list, List<String> selId, String contextPath) {
			return parseAsHtml(list, selId, null, contextPath);
		}
         ...
         * @return
         */
        public String parseAsHtml(List<?> items, List<String> selectedIds, TextGenerator textGenerator, String contextPath) {
            List<Node> nodes = parse(items);

            int beforeLevel = 0;
            StringBuilder html = new StringBuilder();
            String imageFile = "blank-file.png";
            html.append("<ul>");
            ...
            for (int i = 0; i < nodes.size(); i++) {
                ...
                String id = nodes.get(i).getId();
                String text = nodes.get(i).getText();
                boolean isLast = nodes.get(i).isLast();
                ...
                if(!isLast) {
                	imageFile = "notebook.png";
                } else {
                	imageFile = "blank-file.png";
                }
                ...
                String [] lastText = text.split(delimiter);
                html.append(String.format("<li id='%s' data-jstree='{\"icon\" : \""+contextPath+"/resources/images/common/"+imageFile+"\" ,\"selected\": %b}'>%s", elementId, isSelected, lastText[lastText.length-1]));
               // html.append(String.format("<li id='%s' data-jstree='{\"selected\": %b}'>%s", elementId, isSelected, text));
                beforeLevel = level;
            }
